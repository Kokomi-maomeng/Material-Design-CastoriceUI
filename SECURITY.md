...
CastoriceUI v4.0 includes a browser application, application authentication, and a Python/SQLite backend. The supported boundary is:
...
The backend is not designed for direct public exposure. v4.0 supports authenticated password changes but does not implement MFA, out-of-band password reset, fine-grained roles, or multi-node federation. Deployments requiring those controls should place an audited identity-aware access layer in front of the application without bypassing CastoriceUI's session/CSRF checks.
