...
export function TrafficChart({ data }: { data: TrafficPoint[] }) {
  ...
  return <div className="chart chart--traffic"><svg className="native-chart native-chart--interactive" viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="img" tabIndex={0} aria-label={t("上传与下载流量趋势图，可移动鼠标查看数值", "Upload and download traffic trend; move the pointer to inspect values")} onPointerMove={pick} onPointerDown={pick} onPointerLeave={() => setActive(null)} onFocus={() => setActive((value) => value ?? data.length - 1)} onBlur={() => setActive(null)}>
    ...
    {selected ? <g className="chart-inspector" pointerEvents="none"><line x1={selectedX} x2={selectedX} y1={PLOT.y} y2={PLOT.y + PLOT.height} /><rect x={tooltipX} y="22" width="148" height="62" rx="16" /><text x={tooltipX + 12} y="40" className="chart-tooltip-title">{selected.label}</text><circle cx={tooltipX + 14} cy="55" r="4" className="chart-tooltip-dot chart-tooltip-dot--primary" /><text x={tooltipX + 24} y="59">{t("下载", "Download")} {selected.download.toFixed(2)} GB</text><circle cx={tooltipX + 14} cy="72" r="4" className="chart-tooltip-dot chart-tooltip-dot--secondary" /><text x={tooltipX + 24} y="76">{t("上传", "Upload")} {selected.upload.toFixed(2)} GB</text></g> : null}
