...
export function FirstRunConfiguration({ metrics, integrations, onConfigure, onSaveBasics, onComplete }: {
  ...
  return <main className="first-run-page">
    <div className="first-run-header"><div className="auth-brand"><span className="brand-mark"><Icon name="ac_unit" size={26} filled /></span><div><strong>CastoriceUI</strong><span>v2.5</span></div></div><Chip staticChip tone="warning" icon="checklist">{t("首次初始化", "First-run setup")}</Chip></div>
    <section className="first-run-content">
      ...
      <Card variant="outlined"><CardHeader title={t("节点与流量额度", "Node and traffic quota")} description={t("额度使用运营商常见的十进制 GB；周期覆盖不足时总览会明确提示。", "Quota uses decimal GB, as commonly billed by providers. Incomplete cycle coverage is disclosed on the overview.")} />
