...
export function FirstRunConfiguration({ metrics, integrations, onConfigure, onSaveBasics, onComplete }: {
  ...
  const [quotaGb, setQuotaGb] = useState(String(Math.max(1, Math.round(metrics.trafficLimitBytes / 1024 ** 3))));
  ...
  return <main className="first-run-page">
    <div className="first-run-header"><div className="auth-brand"><span className="brand-mark"><Icon name="ac_unit" size={26} filled /></span><div><strong>CastoriceUI</strong><span>v2.3</span></div></div><Chip staticChip tone="warning" icon="checklist">{t("首次初始化", "First-run setup")}</Chip></div>
    <section className="first-run-content">
      ...
      <Card variant="outlined"><CardHeader title={t("节点与流量额度", "Node and traffic quota")} description={t("这两项会成为总览和账号配置额度的唯一显示来源。", "These values become the single display source for the overview and account quotas.")} />
