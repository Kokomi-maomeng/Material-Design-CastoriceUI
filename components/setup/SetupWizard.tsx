...
export function SetupWizard({ selected, status, drafts, onDraft, onClose, onSave }: { selected: IntegrationId | null; status?: IntegrationStatus; drafts: Record<string, Record<string, string>>; onDraft: (id: IntegrationId, field: string, value: string) => void; onClose: () => void; onSave: (id: IntegrationId, values: Record<string, string>) => Promise<void> }) {
  ...
  return <Dialog open title={`${definition.name} 配置向导`} description={definition.summary} onClose={onClose} size="large" actions={<><Button variant="text" onClick={onClose}>{status?.configured || step === 2 ? "完成" : "稍后继续"}</Button>{step > 0 && step < 2 ? <Button variant="outlined" onClick={() => setStep(step - 1)}>上一步</Button> : null}{step === 0 ? <Button trailingIcon="arrow_forward" onClick={() => setStep(1)}>开始配置</Button> : null}{step === 1 ? <Button icon="save" disabled={!requiredComplete || saving} onClick={save}>{saving ? "正在验证" : "保存并验证"}</Button> : null}</>}>
    ...
    {step === 0 ? <div className="wizard-overview"><div className="wizard-outcome"><Icon name={definition.icon} size={28} /><div><span>配置完成后</span><strong>{definition.outcome}</strong></div></div><ol>{definition.steps.map((item) => <li key={item}>{item}</li>)}</ol><div className="wizard-security"><Icon name="shield" /><p>浏览器不会读取服务器配置文件；密码和 Token 只在提交时发送，响应和审计记录不会返回其原文。</p></div></div> : null}
    ...
    {step === 2 ? <div className="wizard-success"><Icon name="check_circle" size={52} filled /><h3>{status?.configured ? "配置已检查" : "参数已保存"}</h3><p>{status?.summary || "后端已接收配置；返回页面后会自动刷新接入状态。"}</p><Chip staticChip tone="success">验证完成</Chip></div> : null}
  </Dialog>;
}
