  ...
  const save = async () => {
    ...
    try {
      ...
      setValidationError(selected === "subscriptions"
        ...
        : selected === "hysteria2"
          ? t("Hysteria2 验证失败，配置未保存。请检查回环 API、服务器 Secret 和协议身份映射。", "Hysteria2 validation failed, so nothing was saved. Check the loopback API, server secret, and protocol-identity mappings.")
