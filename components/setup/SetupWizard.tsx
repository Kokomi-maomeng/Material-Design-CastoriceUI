...
export function SetupWizard({ selected, status, drafts, onDraft, onClose, onSave }: { selected: IntegrationId | null; status?: IntegrationStatus; drafts: Record<string, Record<string, string>>; onDraft: (id: IntegrationId, field: string, value: string) => void; onClose: () => void; onSave: (id: IntegrationId, values: Record<string, string>) => Promise<IntegrationStatus> }) {
  ...
  const [lastSelected, setLastSelected] = useState<IntegrationId | null>(selected);
  const previousSelected = useRef<IntegrationId | null>(selected);
  const renderedSelected = selected ?? lastSelected;
  const definition = integrationDefinitions.find((item) => item.id === renderedSelected);
  ...
  useEffect(() => {
    let frame = 0;
    if (selected && (previousSelected.current === null || previousSelected.current !== selected)) frame = window.requestAnimationFrame(() => {
  ...
  if (!definition || !renderedSelected) return null;
  const local = (value: { zh: string; en: string }) => withoutTerminalPeriod(value[language]);
  const values = drafts[renderedSelected] ?? {};
  const resolvedValues = Object.fromEntries(definition.fields.map((field) => [field.id, values[field.id] ?? field.options?.[0]?.value ?? ""]));
  const isProtocol = ["hysteria2", "anytls", "vless", "socks5", "shadowsocks", "vmess", "trojan", "tuic"].includes(renderedSelected);
  const requiredComplete = definition.fields.filter((field) => field.required).every((field) =>
    resolvedValues[field.id]?.trim() || (renderedSelected === "subscriptions" && field.id === "baseUrl" && status?.configured),
  );
  const save = async () => {
    setSaving(true);
    setValidationError("");
    try {
      const verified = await onSave(renderedSelected, resolvedValues);
      setResult(verified);
      setStep(2);
    } catch (caught) {
