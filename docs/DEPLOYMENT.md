# CastoriceUI v1.3 deployment / 部署配置手册
...
Expected health version for this release is `1.3.0`. The unit uses a dedicated account, `NoNewPrivileges`, read-only system protection, bounded memory, and a writable SQLite directory only.
...
curl -u admin https://panel.example.com/api/v1/health      # expect 200 and 1.3.0
